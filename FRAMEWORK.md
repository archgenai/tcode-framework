...
0. **Ideate & Scaffold** — Use the `/new-project` slash command (Claude Code). It is the
   single interface for going from idea to scaffolded project. It runs Prompt Zero planning
   first (Step 0), gets mandatory developer approval, then writes the full project directory
   in one session. See **§ Prompt Zero** and `.claude/commands/new-project.md` for the full
   protocol.
   *Legacy:* `tools/prompt-zero/` web UI, `templates/PROMPT_ZERO.md` manual fill, and
   `python new_project.py` are terminal-only fallbacks — use them only outside a Claude Code
   session.
1. **Specify** — Review `APP_SPEC.md` (generated by `/new-project`) and adjust if needed.
2. **Bootstrap** — Review `CLAUDE.md` and `REQUIREMENTS.md` (generated by `/new-project`) before implementation.
3. **Build** — Work phase by phase. Never implement beyond the current phase.
4. **Iterate** — After each phase: update `STACK.md`, commit, update memory.
...
| `.claude/commands/new-project.md` | Agent | `/new-project` — primary interface: idea → Prompt Zero → scaffold → register (one session) |
| `new_project.py` | Developer | **Legacy.** Terminal-only scaffold wizard — use only outside a Claude Code session |
